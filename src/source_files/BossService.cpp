...
void BossService::createBoss(vector<shared_ptr<Person>> &people, int id) {
    ...
    BossInitializationData data(id, name, age, firstName, lastName, title, move(address));
...
void BossService::giveRaise(vector<shared_ptr<Person>>& people) {
    ...
    dynamic_pointer_cast<Boss>(people[bossIndex])->giveRaise(dynamic_pointer_cast<Employee>(people[employeeIndex]));
}
